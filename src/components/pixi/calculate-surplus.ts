...
export const calculateSurpluses = ({
    ...
    const consumerSurplus =
        demandIntegral(demand.range.quantityMin, quantity) - price * (quantity - demand.range.quantityMin);
    const producerSurplus =
        price * (quantity - supply.range.quantityMin) - supplyIntegral(supply.range.quantityMin, quantity);
